defmodule ChallengeTest do
  ...
  describe "create_users/2" do
    ...
    test "creates users", %{server: srv} do
      assert Challenge.create_users(srv, ["user1", "user2"]) == :ok
    end

    test "ignores empty user", %{server: srv} do
      assert Challenge.create_users(srv, [""]) == :ok
      bet = %{
        user: "",
  ...
  describe "bet/2" do
    ...
    test "debits bets from users", %{srv: srv} do
      assert Challenge.create_users(srv, ["user1"]) == :ok
      bet = %{
        user: "user1",
        currency: "USD",
        transaction_uuid: "16d2dcfe-b89e-11e7-854a-58404eea6d16",
    ...
    test "returns error on insufficient balance", %{srv: srv} do
      assert Challenge.create_users(srv, ["user2"]) == :ok
      bet = %{
        user: "user2",
        currency: "USD",
        transaction_uuid: "16d2dcfe-b89e-11e7-854a-58404eea6d16",
    ...
    test "returns same OK response on duplicate transaction with same data", %{srv: srv} do
      assert Challenge.create_users(srv, ["duper1"]) == :ok
      bet = %{
        user: "duper1",
        currency: "USD",
        transaction_uuid: "16d2dcfe-b89e-11e7-854a-58404eea6d16",
      ...
      orig_result = Challenge.bet(srv, bet)
  ...
  describe "win/2" do
    ...
    test "credits win for user bet", %{srv: srv} do
      assert Challenge.create_users(srv, ["winner1"]) == :ok
      bet = %{
        user: "winner1",
        currency: "USD",
        transaction_uuid: "16d2dcfe-b89e-11e7-854a-58404eea6d16",
    ...
    test "returns error on bet with an existing win", %{srv: srv} do
      assert Challenge.create_users(srv, ["winner2"]) == :ok
      bet = %{
        user: "winner2",
        currency: "USD",
      ...
      result = Challenge.win(srv, notwin)
      assert result.status == "RS_ERROR_UNKNOWN"
    end
  end
end
